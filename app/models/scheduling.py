...
class JobScheduleRequest(BaseModel):
    ...
    job_name: Optional[str] = Field(
        None, description="Optional name/identifier for the job"
    )
    priority: Priority = Field(Priority.LOW, description="Priority level of the job")
...
class TimeSlot(BaseModel):
    ...
    carbon_intensity: int = Field(
        ..., description="Average carbon intensity for the slot (gCO2/kWh)"
    )
...
class SchedulingMetadata(BaseModel):
    ...
    forecast_confidence: str = Field(
        ..., description="Confidence level in the forecast"
    )
    cached_data_age_minutes: int = Field(
        ..., description="Age of the cached data in minutes"
    )
...
class JobScheduleResponse(BaseModel):
    ...
    optimal_start_time: datetime = Field(
        ..., description="Optimal start time for the job (UTC)"
    )
    optimal_end_time: datetime = Field(
        ..., description="Optimal end time for the job (UTC)"
    )
    carbon_intensity: int = Field(
        ..., description="Average carbon intensity for the optimal slot (gCO2/kWh)"
    )
    carbon_index: CarbonIndex = Field(..., description="Carbon intensity category")
