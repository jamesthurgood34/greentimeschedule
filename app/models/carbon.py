...
class CarbonIntensityPeriod(BaseModel):
    ...
    intensity_forecast: int = Field(..., description="Forecast carbon intensity (gCO2/kWh)")
    intensity_actual: Optional[int] = Field(None, description="Actual carbon intensity if available")
...
class CarbonForecast(BaseModel):
    ...
    forecast_periods: List[CarbonIntensityPeriod] = Field(..., description="List of carbon intensity periods")
    data_freshness: datetime = Field(..., description="When this forecast data was last updated")
